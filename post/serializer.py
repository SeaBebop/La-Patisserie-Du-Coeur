...
class PostSerializer(serializers.ModelSerializer):
    ...
    def get_product_image(self,instance):
            my_list = Product.objects.filter(id=instance.id).values_list('image',flat=True)
            return '/products/' + my_list[0]
    class Meta:
        model = Product
        fields = ('name','product_image','category','description', 'slug', 'price',
        'quantity','image','id')
        lookup_field = 'id'
        extra_kwargs = {
            'url': {'lookup_field': 'id'}
    ...
    def update(self, instance, validated_data):
        ...
        # Perform any additional custom logic if needed
