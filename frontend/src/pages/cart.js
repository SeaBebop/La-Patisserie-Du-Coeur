...
const ORDER_ID_URL = '/api/v1/order/'
const CREATE_CHECKOUT_URL = '/api/v1/checkout/create-checkout-session/'
...
const Cart = () => {
    ...
    const deleteSubmit = async (e) => {
        ...
        await sleep(10)
        document.getElementById('urlCheckout').click()
    }
    
    const cartCheckoutPost = async (e) =>{
        e.preventDefault();
        const sessionKey = data[0]?.session_key ? data[0]?.session_key : ''
        const userID = access_token ? access_token : ''

        try{
            const response = await axios.post(CREATE_CHECKOUT_URL,JSON.stringify({'sessionKey':sessionKey,'userID':userID}), {
        ...
        return (<div>
            {data ?
                ...
                data.slice(start, end).map(datas =>
                        ...
                        <div key={datas.id} className=" flex uppercase text-[3.5vw] lg:text-[1.25vw] text-[#4d3526] bg-[#ff6e61a9] flex-row  pt-[1vw] 
                         justify-around h-auto lg:h-[10vw] w-[] lg:w-[60vw] lg:pt-[.4vw] before:border-t before:absolute lg:before:w-[55%] before:w-[65%] before:mt-[-.6vw]">
                            <div className=" flex  gap-[3vw] pt-[.5vw] ">
                                ...
                                <img src={"static" + datas.product_image} loading="lazy" ref={containerRef} className="lg:h-[7.5vw] h-[14vw]  rounded-sm shadow-md" alt="" />
                    ...
                    :
                    <div className="lg:h-[30vw] h-[45vw] mt-[.5vw] flex flex-col w-[60vw] shadow-2xl bg-[#ff6e61a9] rounded-sm ">
                        <div className="h-[25%] w-[97%] ml-[.6vw] mt-[1.5vw] shadow-lg rounded-sm bg-slate-300 animate-pulse"></div>
                        <div className="h-[25%] w-[97%] ml-[.6vw] mt-[1.5vw] shadow-lg rounded-sm bg-slate-300 animate-pulse"></div>
                        <div className="h-[25%] w-[97%] ml-[.6vw] mt-[1.5vw] shadow-lg rounded-sm bg-slate-300 animate-pulse"></div>
                    </div>
            }
    ...
    return (
        <div id='cart' className="lg:h-[110vw] min-h-[850px]">
            ...
            <div className="lg:h-auto h-[100vh] short:min-h-[757px] justify-center lg:flex-row  flex flex-col  items-center font-body gap-[4vw] lg:gap-3  pt-[2vw] w-screen overflow-hidden">
                <div className="lg:w-[60vw] w-[95vw] overflow-x-hidden  mr-[0vw] lg:mr-[3.6vw] flex  flex-col ">
                    ...
                    {data
                        ...
                        <HandlePages />  : isEmpty === true
                            ...
                            :
                            <div className="lg:h-[30vw] h-[45vw] mt-[.5vw] flex flex-col shadow-2xl bg-[#ff6e61a9] rounded-sm ">
                                <div className="h-[25%] w-[97%] ml-[.6vw] mt-[1.5vw] shadow-lg rounded-sm bg-slate-300 animate-pulse"></div>
                                <div className="h-[25%] w-[97%] ml-[.6vw] mt-[1.5vw] shadow-lg rounded-sm bg-slate-300 animate-pulse "></div>
                                <div className="h-[25%] w-[97%] ml-[.6vw] mt-[1.5vw] shadow-lg rounded-sm bg-slate-300 animate-pulse"></div>
                            </div>
                ...
                {data ?
                    <div className="  uppercase flex justify-center flex-col h-[19vw] text-[4vw] lg:text-[1.4vw] lg:mt-[2vw] bg-[#FF6F61] text-[#4d3526] w-[55vw] lg:w-[20vw] p-[5vw] shadow-lg rounded-sm ">
                ...
                */}
                            ...
                            <form onClick={cartCheckoutPost} method="POST">
                                <input type="submit" id='urlCheckout' name=""  hidden value="CHECK OUT" />
                            </form>

                <a className={buildingUrl == false ? "pointer-events-none text-[3vw] lg:text-[1.2vw]  h-[7vw] lg:h-[2.5vw] w-[19vw] lg:w-[8vw]  hover:translate-y-[.1vw] transition-all delay-75 border rounded-md bg-green-500 p-[.5vw] mt-[.3vw] text-black"   : "cursor-pointer text-[3vw] lg:text-[1.2vw]  h-[7vw] lg:h-[2.5vw] w-[18vw] lg:w-[8vw] hover:bg-[#1f5cacd0] hover:translate-y-[.1vw] transition-all delay-75 border rounded-md bg-blue-500 p-[.5vw] mt-[.3vw] text-black"}  id='urlCheckout' href={buildingUrl !=false ? url : ''}> {buildingUrl != false ? "Checkout": "Building..."}</a>
                        </div>
                    </div>
                    :
                    isEmpty === true ?
