...
import QuantityInput from '../components/IncDec';
import MessageExampleCompact from '../components/Prompt';
import MessageFixedRight from '../components/PromptLine';
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';

const PRODUCT_DATA_URL = '/api/v1/shop';
const ORDER_URL = '/api/v1/order/';

const notify = (messageVariable,booleanError) => {
  const message = messageVariable
  const error = booleanError 
  error == false ? toast.error(message): toast(message)
}
...
const Products = () => {
  ...
  const onOrderSubmit = async (e) => {
    ...
    const targetValueQuantity = e.target[1].value != "" ? e.target[1].value : 0;
    const targetID = String(e.target.className);
    ...
    return (
      ...
      (productType !== 'All' ?
        ...
        <div>
          ...
          <div className='flex flex-col lg:flex-row  mb-[2vw] gap-[.5vw]'>
            {data ?
              data.filter(datas => datas.category === productType).slice(start, end).map(datas =>
                /*Key is portional to the backend name*/
                <div key={datas.id} className="flex lg:h-[25vw] h-[53.5vw] flex-col  justify-center  lg:text-[1.4vw] shadow-lg bg-slate-100 outline outline-[.1vw] outline-slate-300 px-[1.3vw] py-[.8vw] rounded-[.5vw] mx-[1vw] my-[1vw] items-center">

                  <img className='lg:h-[9.5vw] h-[22vw] shadow-xl rounded-sm unrevealed ' ref={containerRef} src={"static/" + datas.product_image} alt="" />  <p className=' font-Body md:text-[2vw] lg:text-[1.15vw] uppercase'>{datas.name}</p> <p className='font-body text-[3.5vw] lg:text-[1.34vw]'>${datas.price}</p> <p className='font-body md:text-[1.8vw] lg:text-[1.2vw]'>{datas.quantity} LEFT</p>


                  <form onSubmit={onOrderSubmit} className={datas.id} >
                    <div className='flex lg:flex-col flex-row'>
                      <QuantityInput total={datas.quantity} name={datas.id} />

                      <button

                        className='bg-green-400 shadow-md hover:bg-green-200 ml-[11vw] flex items-center justify-center font-medium lg:mt-[1vw] mt-[-5vw] outline outline-slate-200 outline-[.05vw] rounded-sm  px-[.3vw] py-[.2vw] text-[2.5vw] lg:text-[1.1vw] '
        ...
        <div className='lg:h-[33vw]'>
          <div className='flex flex-col lg:flex-row  mb-[2vw] gap-[.5vw]'>
            {data ?
              data.slice(start, end).map(datas =>
                /*Key is portional to the backend name*/
                <div key={datas.id} className="flex lg:h-[26vw] h-[53.5vw] flex-col  justify-center  lg:text-[1.4vw] shadow-lg bg-slate-100 outline outline-[.1vw] outline-slate-300 px-[1.3vw] py-[.8vw] rounded-[.5vw] mx-[1vw] my-[1vw] items-center">

                  <img className='lg:h-[9.5vw] h-[22vw] shadow-xl rounded-sm unrevealed ' ref={containerRef} src={"static/" + datas.product_image} alt="" />  <p className='md:text-[2vw] lg:text-[1.15vw] font-Body uppercase'>{datas.name}</p> <p className='font-body text-[3.5vw] lg:text-[1.2vw]'>${datas.price}</p> <p className='font-body md:text-[1.8vw] lg:text-[1.2vw]'>{datas.quantity} LEFT</p>
                  <form onSubmit={onOrderSubmit} className={datas.id} >
                    <div className='flex lg:flex-col flex-row'>
                      <QuantityInput total={datas.quantity} name={datas.id} />
          ...
          <div className='flex justify-center mb-[1vw]  gap-[3vw] lg:gap-[.5vw]'>
            {
              pageAmount &&
  ...
  return (
    ...
    <div className='lg:h-[82vw] h-auto min-h-[800px]'>
      <div className='flex justify-center  overflow-hidden'>
        <img className=' absolute min-w-[100vw] max-h-[75vw] mt-[-29vw] z-[3]  ' loading='lazy' src="static/just-food-sharpen.webp" alt="" />
                ...
                data ?
                  ...
                  <div>
                    ...
                    <div id='desktop' className='w-[90vw]  lg:h-[30vw] h-[40vw] hidden lg:flex flex-col item-center  animate-pulse'>
                      <div className=' flex flex-row w-[100%] h-[100%]  mb-[2vw]'>
                      ...
                      <div className='mt-[-1.5vw] h-[10%] ml-[34vw] w-[25%] flex  mb-[1vw] rounded-md bg-slate-200'>
                    ...
                    <div id='mobile' className='w-[90vw]  min-h-[600px] max-h-[600px] h-[30vw]  lg:hidden flex-col item-center  animate-pulse'>
                      <div className=' flex flex-col justify-center items-center w-[100%]  mb-[2vw] h-[100%]'>
                        <div className='mt-[4vw] h-[25%]  w-[85%] flex rounded-md bg-slate-200'></div>
                        <div className='mt-[4vw] h-[25%]  w-[85%] flex rounded-md bg-slate-200'></div>
                        <div className='mt-[4vw] h-[25%]  w-[85%] flex rounded-md bg-slate-200 '></div>

                        <div className='mt-[4.5vw] h-[8%]  w-[55%] flex  mb-[1vw] rounded-md bg-slate-200'>
                        </div>
