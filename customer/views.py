...
class Customer(APIView):
        ...
        if request.user.is_authenticated:
            if request.user.customer_id == None:
                try:
                    ...
                    return Response('User Creation Library Failed',status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def get(self, request):
        ...
        if request.user.is_authenticated:
            ...
            return Response("No customer data found",status.HTTP_401_UNAUTHORIZED)
...
class CustomerPurchase(APIView):
    ...
    def get(self, request):
        ...
        paymentComprehension = [{'payment_intent': d['payment_intent'], 'customerID':d['customer'], 'checkoutID':d['id'], 'productsName': [Product.objects.filter(id=(int(x))).values_list('name', flat=True)[0] for x in d['metadata']['product'].split(',')],
                                 ...
                                 'productImages': [ '/products/'+ Product.objects.filter(id=(int(x))).values_list('image', flat=True)[0] for x in d['metadata']['product'].split(',')],
